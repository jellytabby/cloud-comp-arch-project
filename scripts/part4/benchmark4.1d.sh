...
MPSTAT_CMD="mpstat 1 | while read line; do echo \"\$(date +%s%3N) \$line\"; done"
...
T=3
    for C in 1 2 3; do
        ...
        gcloud compute ssh --ssh-key-file ~/.ssh/cloud-computing --zone europe-west1-b "${MEMCACHED_SERVER_NODE}" --command "echo 'mpstat 1 | while read line; do echo \$(date +%s%3N) \$line; done | tee ~/mpstat_T${T}_C${C}.txt' > ~/run_mpstat.sh && TERM=xterm-256color tmux new-session -d bash ~/run_mpstat.sh"
        ...
        for reps in {1..3}; do
            ...
            gcloud compute scp --ssh-key-file ~/.ssh/cloud-computing --zone europe-west1-b "${MEMCACHED_SERVER_NODE}:~/mpstat_T${T}_C${C}.txt" "${OUTPUT_DIR}/mpstat_T${T}_C${C}_rep${reps}.txt"
        done
    done
# done