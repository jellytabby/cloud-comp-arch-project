...
# 2. Barnes (low CPU interference) can be on node-b-4core with 4 threads (but reduce threads to 3 to leave some cores for other jobs)
# 3. Blackscholes (low CPU interference) can be on node-b-4core with 4 threads (but reduce threads to 3 to leave some cores for other jobs)
# 4. Canneal (low CPU interference) can be on node-b-4core with 4 threads (but reduce threads to 3 to leave some cores for other jobs)
# 5. Freqmine (high CPU and memory usage) should be on node-a-8core with 8 threads
# 6. Streamcluster (high CPU and memory usage) should be on node-a-8core with 8 threads
# 7. Ensure that memcached (which is always on node-a-8core) is not interfered with
declare -A barnes_map=(["nodetype"]="node-b-4core" ["threads"]="3" ["cpus"]="0-2")  # Low CPU interference, reduced threads to 3 to allow more jobs on node-b-4core
declare -A blackscholes_map=(["nodetype"]="node-b-4core" ["threads"]="3" ["cpus"]="0-2")  # Low CPU interference, reduced threads to 3 to allow more jobs on node-b-4core
declare -A canneal_map=(["nodetype"]="node-b-4core" ["threads"]="3" ["cpus"]="0-2")  # Low CPU interference, reduced threads to 3 to allow more jobs on node-b-4core
declare -A freqmine_map=(["nodetype"]="node-a-8core" ["threads"]="8" ["cpus"]="0-7")  # High CPU and memory usage, kept 8 threads
declare -A radix_map=(["nodetype"]="node-a-8core" ["threads"]="8" ["cpus"]="0-7")  # Low memory and CPU interference, kept 8 threads
declare -A streamcluster_map=(["nodetype"]="node-a-8core" ["threads"]="8" ["cpus"]="0-7")  # High CPU and memory usage, kept 8 threads
declare -A vips_map=(["nodetype"]="node-b-4core" ["threads"]="3" ["cpus"]="1-3")  # Low memory and CPU interference, kept 3 threads
