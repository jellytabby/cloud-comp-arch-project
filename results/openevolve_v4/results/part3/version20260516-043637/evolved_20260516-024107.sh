...
# 2. Barnes (low CPU interference) can be on node-b-4core with 4 threads (0-3 cores)
# 3. Blackscholes (low CPU interference) can be on node-b-4core with 4 threads (0-3 cores)
# 4. Canneal (low CPU interference) can be on node-b-4core with 4 threads (0-3 cores)
# 5. Freqmine (high CPU and memory usage) should be on node-a-8core with 8 threads (0-7 cores)
# 6. Streamcluster (high CPU and memory usage) should be on node-a-8core with 8 threads (0-7 cores)
# 7. Ensure that memcached (which is always on node-a-8core) is not interfered with
# 8. Radix and Vips (low memory and CPU interference) can share node-a-8core with 4 threads each (to utilize all 8 cores)
# 9. To avoid interference between high-CPU jobs, stagger their start times
# 10. Vips (low memory and CPU) can be scheduled after streamcluster to utilize node-a-8core's remaining resources
...
declare -A radix_map=(["nodetype"]="node-a-8core" ["threads"]="4" ["cpus"]="0-3")  # Low memory and CPU interference (shared with Vips)
declare -A vips_map=(["nodetype"]="node-a-8core" ["threads"]="4" ["cpus"]="4-7")  # Low memory and CPU, after streamcluster
